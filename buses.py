...
def distance(p1: Coord, p2: Coord) -> float:
    ...
    #Diferències entre latitud i longitud
    ...
    #Calculem la distància i la retornem en metres
    return radi * c * 1000
        ...
        for parada in linia['Parades']['Parada']:
            ...
            if parada['Municipi'] == 'Barcelona':
                ...
                if first:
                    ...
                    graf.add_edge(parada['CodAMB'], parada_antiga, dist=distance(dict(graf.nodes)[parada['CodAMB']]['pos'], dict(graf.nodes)[parada_antiga]['pos']), pos1=dict(graf.nodes)[parada['CodAMB']]['pos'], pos2=dict(graf.nodes)[parada_antiga]['pos'])
...
def plot(g: BusesGraph, nom_fitxer: str) -> None:
    ...
    for edge in dict(g.edges).values():
        if edge['dist'] < 10000: #si la distancia de l'aresta és de més de 10km, no la volem mostrar ja que tan sols representa les unions entre els dos extrems de linia
