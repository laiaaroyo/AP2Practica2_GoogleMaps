...
class Billboard: 
    ...
    cinemas: list[Cinema] # Sales de cine disponibles
    projections: list[Projection] # Pellícules en la seva corresponent sala, juntament amb la seva hora de projecció.
    
    def cerca_peli_per_nom(self,keyword:str) -> list[Projection]:
        """Donat un nom d'una pel·lícula ens retorna una llista de projeccions amb els cinemes
        que ens mostren la pel·lícula,els horaris,.... Si no hi ha cap cerca que coincideixi en cap
        cine retorna una llista buida"""
       
        return [projection for projection in self.projections if keyword in projection.film.title]

    def cerca_peli_per_genere(self,genre:str) -> list[Projection]:
        """Seleccionem les pel·lícules del gènere que ens interessa. Retornem una llista de projeccions"""
        
        return [projection for projection in self.projections if genre in projection.film.genre]
    ...
    def cerca_peli_per_director(self,director:str) -> list[Projection]:
        ...
        return [projection for projection in self.projections if director in projection.film.director]
    ...
    def cerca_peli_per_actor(self,actor:str) -> list[Projection]:
        ...
        return [projection for projection in self.projections if actor in projection.film.actors]
...
def read() -> Billboard:
    ...
    for url in urls:
        ...
        if response:
            ...
            return Billboard(list(films.values()), cinemes, projections)


def read_list_cinemas(contingut:BeautifulSoup, cinemes:dict[str,str] ) -> dict[str,str]:
...
def read_list_films(contingut:BeautifulSoup,films:dict[str,Film]) -> dict[str,Film]:
    ...
    for pelicula_actual in dia_actual:
        ...
        for entry in data: 
            ...
            data_movie = json.loads(data_movie) # Ho convertim en un diccionari
...
def read_list_projections(contingut: BeautifulSoup,cinemes:dict[str,str],films:dict[str,Film],projections:list[Projection]) -> list[Projection]:
    ...
    for cine in cines:
        for projeccio in cine:
            ...
            cinema_actual = Cinema(name,cinemes[name])
